...
1. Определи категорию письма. Выбери ОДНУ из:
   ...
   **TENDER** — письмо о тендере, закупке, аукционе:
   - Извещения с номерами (0123456789...)
   - Приглашения на электронные аукционы (44-ФЗ, 223-ФЗ)
   - Уведомления от ЭТП (Сбербанк-АСТ, Росэлторг, ЕЭТП)
   - Письма с НМЦК, сроками подачи заявок
   ⚠️ НЕ классифицируй как TENDER:
   - Новостные дайджесты с подборкой тендеров
   - Рекламные рассылки ЭТП
   - Общие уведомления ГИС ЕИС без конкретных данных

   **SPAM** — нежелательная реклама БЕЗ полезного содержания:
   - Массовые рассылки "заработай миллион", "инвестиции"
   - Коммерческие предложения от неизвестных отправителей
   - Письма с призывом "срочно перейдите по ссылке"
   - Чистая реклама товаров/услуг без контекста
   ⚠️ НЕ классифицируй как SPAM:
   - Информационные письма от авиакомпаний, банков, магазинов
   - Уведомления о бронированиях, заказах, покупках
   - Деловые предложения от известных партнёров
   - Письма с полезной информацией + рекламные блоки (это GENERAL)

   **GENERAL** — обычная деловая переписка и уведомления:
   - Рабочие письма от коллег/партнёров
   - Информационные письма от компаний (авиакомпании, банки, магазины)
   - Подтверждения бронирований, заказов, покупок
   - Чеки, квитанции, билеты
   - Уведомления о встречах, событиях
   - Письма с полезной информацией (даже если есть рекламные блоки)
   - Личная переписка

   **EMPTY** — пустое или бессмысленное письмо:
   - Без темы и текста
   - Только подпись или автоответ
   - Технический мусор
...
async def test_llm_analyzer():
    ...
    # Тест 5: Информационное письмо от авиакомпании (раньше было SPAM)
    print("Тест 5: Информационное письмо от авиакомпании")
    result = await analyzer.analyze_email(
        subject="Детали бронирования VBPZ6G",
        body_text="Уважаемый клиент! Недавно вы приобрели авиабилет. Вылет: Нальчик - Москва, рейс DP6902. Багаж не включён в ваш тариф.",
        from_email="noreply@flypobeda.ru"
    )
    print(f"  Категория: {result.get('category')}")
    print(f"  Резюме: {result.get('summary')}")
    print()
